...
--  Created Date                : Wed Oct 01 18:56:26 ART 2014
--  Modified Date               : Wed Oct 01 18:56:26 ART 2014
--  Created By                  : owb11204_idesa
--  Modified By                 : owb11204_idesa
--  Generated Object Type       : PL/SQL Package
--  Generated Object Name       : "APP_STG_MP_USUARIOS_PAGO"
--  Comments                    : 
*****************************************************************************/
...
BEGIN
  ...
  sql_stmt := 'SELECT TABLE_NAME FROM OWB$TEMP_TABLES WHERE PACKAGE_NAME = ''APP_STG_MP_USUARIOS_PAGO''';
  ...
  END LOOP;
  CLOSE c1;
  EXECUTE IMMEDIATE 'DELETE FROM OWB$TEMP_TABLES WHERE PACKAGE_NAME = ''APP_STG_MP_USUARIOS_PAGO''';
  COMMIT;
EXCEPTION WHEN OTHERS THEN
  DBMS_OUTPUT.PUT_LINE(SQLERRM);
END;  
...
END;
/
CREATE OR REPLACE PACKAGE "APP_STG_MP_USUARIOS_PAGO"  AS

-- Map Generation Timestamp
get_generation_date      VARCHAR2(100) := 'Wed Oct 01 18:56:26 ART 2014';
...
get_lob_uoid                  CONSTANT VARCHAR2(40) := '04641062EBF92D98E053B200090AD236';
get_model_name                CONSTANT VARCHAR2(40) := '"APP_STG_MP_USUARIOS_PAGO"';
...
"STG_MP_USUARIOS_PAGO_St" Boolean;
...
-- Function Main -- Entry point in package "APP_STG_MP_USUARIOS_PAGO"
--------------------------------------------------
-- Function Main: Should only be called by OWB. --
--------------------------------------------------

FUNCTION Main("CANT_FILAS" IN NUMBER
 DEFAULT NULL,
...
-------------------------------------------------
PROCEDURE Main(p_status OUT VARCHAR2,               "CANT_FILAS" IN NUMBER
 DEFAULT NULL,
"TABLE_NAME" IN VARCHAR2
 DEFAULT NULL,
"COD_FECHA" IN NUMBER
...
END "APP_STG_MP_USUARIOS_PAGO";


/

CREATE OR REPLACE PACKAGE BODY "APP_STG_MP_USUARIOS_PAGO" AS

-- Define cursors here so that they have global scope within the package (for debugger)

---------------------------------------------------------------------------
--
-- "EXT_MP_USUARIOS_PAGO_c" Cursor declaration 
--
---------------------------------------------------------------------------
CURSOR "EXT_MP_USUARIOS_PAGO_c" IS
 SELECT
/* EXT_MP_USUARIOS_PAGO.OUTGRP1 */
  "EXT_MP_USUARIOS_PAGO"."ID_CLIENTE" "ID_CLIENTE",
  "EXT_MP_USUARIOS_PAGO"."PIN" "PIN",
  "EXT_MP_USUARIOS_PAGO"."ESTADO" "ESTADO"
FROM
  "EXT_MP_USUARIOS_PAGO"  "EXT_MP_USUARIOS_PAGO"; 

---------------------------------------------------------------------------
--
-- "EXT_MP_USUARIOS_PAGO_c$1" Cursor declaration 
--
---------------------------------------------------------------------------
CURSOR "EXT_MP_USUARIOS_PAGO_c$1" IS
 SELECT
/* EXT_MP_USUARIOS_PAGO.OUTGRP1 */
  "EXT_MP_USUARIOS_PAGO"."ID_CLIENTE" "ID_CLIENTE",
  "EXT_MP_USUARIOS_PAGO"."PIN" "PIN",
...
---------------------------------------------------------------------------
-- Function "STG_MP_USUARIOS_PAGO_Bat"
...
BEGIN
  ...
  get_abort_procedure := FALSE;
    ...
    batch_auditd_id := OWBSYS.WB_RT_MAPAUDIT.auditd_begin(  -- Template BatchAuditDetailBegin
      ...
      p_step=>0,
      p_name=>'"STG_MP_USUARIOS_PAGO_Bat"',
      p_source=>'*',
      p_source_uoid=>'*',
      p_target=>'"STG_MP_USUARIOS_PAGO"',
      p_target_uoid=>'04641062EBF22D98E053B200090AD236',
    ...
    p_parent_operator_uoid=>'04641062EC9C2D98E053B200090AD236', -- Operator EXT_MP_USUARIOS_PAGO
    p_parent_object_name=>'EXT_MP_USUARIOS_PAGO',
    p_parent_object_uoid=>'04641062EBA72D98E053B200090AD236', -- Tabla Externa EXT_MP_USUARIOS_PAGO
    p_parent_object_type=>'Tabla Externa',
    p_object_name=>'EXT_MP_USUARIOS_PAGO',
    p_object_uoid=>'04641062EBA72D98E053B200090AD236', -- Tabla Externa EXT_MP_USUARIOS_PAGO
    p_object_type=>'Tabla Externa',
    p_location_uoid=>'03C05C514D115B62E053B200090A5929' -- Location DW_STG_LOCATION1
  );
    	get_audit_detail_type_id := OWBSYS.WB_RT_MAPAUDIT.register_audit_detail_type(
    p_rtd=>get_audit_detail_id,
    p_parent_operator_uoid=>'04641062EBF22D98E053B200090AD236', -- Operator STG_MP_USUARIOS_PAGO
    p_parent_object_name=>'STG_MP_USUARIOS_PAGO',
    p_parent_object_uoid=>'04641062EBDD2D98E053B200090AD236', -- Tabla STG_MP_USUARIOS_PAGO
    p_parent_object_type=>'Tabla',
    p_object_name=>'STG_MP_USUARIOS_PAGO',
    p_object_uoid=>'04641062EBDD2D98E053B200090AD236', -- Tabla STG_MP_USUARIOS_PAGO
    p_object_type=>'Tabla',
    p_location_uoid=>'03C05C514D115B62E053B200090A5929' -- Location DW_STG_LOCATION1
  );
  ...
  BEGIN
    ...
    IF NOT "STG_MP_USUARIOS_PAGO_St" THEN
      IF get_use_hc THEN
      ...
      ELSE
        ...
        EXECUTE IMMEDIATE 'TRUNCATE TABLE "STG_MP_USUARIOS_PAGO"';
      END IF;
    END IF;
    ...
    IF NOT "STG_MP_USUARIOS_PAGO_St" THEN
      ...
      INSERT
      /*+ APPEND PARALLEL("STG_MP_USUARIOS_PAGO") */
      INTO
        "STG_MP_USUARIOS_PAGO"  "STG_MP_USUARIOS_PAGO"
        ("ID_CLIENTE",
        "PIN",
        "ESTADO",
        "COD_FECHA_DIARIO",
...
/* EXT_MP_USUARIOS_PAGO.OUTGRP1 */
  ...
  "EXT_MP_USUARIOS_PAGO"."PIN" "PIN",
  "EXT_MP_USUARIOS_PAGO"."ESTADO" "ESTADO",
  "APP_STG_MP_USUARIOS_PAGO"."COD_FECHA_DIARIO"/* ATTRIBUTE P_COD_FECHA_DIARIO.OUTGRP1.COD_FECHA_DIARIO */ "COD_FECHA_DIARIO$1",
  "APP_STG_MP_USUARIOS_PAGO"."COD_FECHA"/* ATTRIBUTE P_COD_FECHA.OUTGRP1.COD_FECHA */ "COD_FECHA$1"
FROM
  "EXT_MP_USUARIOS_PAGO"  "EXT_MP_USUARIOS_PAGO"
        )
      ;
  ...
  END IF;
  RETURN TRUE;
END "STG_MP_USUARIOS_PAGO_Bat";



-- Procedure "EXT_MP_USUARIOS_PAGO_p" is the entry point for map "EXT_MP_USUARIOS_PAGO_p"

PROCEDURE "EXT_MP_USUARIOS_PAGO_p"
("CANT_FILAS" IN NUMBER
 DEFAULT NULL,
"TABLE_NAME" IN VARCHAR2
 DEFAULT NULL,
...
get_map_name               CONSTANT VARCHAR2(40) := '"EXT_MP_USUARIOS_PAGO_p"';
get_source_name            CONSTANT VARCHAR2(2000) := SUBSTRB('"EXT_MP_USUARIOS_PAGO"',0,2000);
get_source_uoid            CONSTANT VARCHAR2(2000) := SUBSTRB('04641062EC9C2D98E053B200090AD236',0,2000);
...
"STG_MP_USUARIOS_PAGO_id" NUMBER(22) := 0;
"STG_MP_USUARIOS_PAGO_ins" NUMBER(22) := 0;
"STG_MP_USUARIOS_PAGO_upd" NUMBER(22) := 0;
"STG_MP_USUARIOS_PAGO_del" NUMBER(22) := 0;
"STG_MP_USUARIOS_PAGO_err" NUMBER(22) := 0;
...
"STG_MP_USUARIOS_PAGO_srk" OWBSYS.WB_RT_MAPAUDIT.NUMBERLIST;
...
"STG_MP_USUARIOS_PAGO_ir"  index_redirect_array;
"SV_STG_MP_USUARIOS_PAGO_srk" NUMBER;
"STG_MP_USUARIOS_PAGO_new"  BOOLEAN;
"STG_MP_USUARIOS_PAGO_nul"  BOOLEAN := FALSE;
...
"EXT_MP_USUARIOS_PAGO_si" NUMBER(22) := 0;

"EXT_MP_USUARIOS_PAGO_i" NUMBER(22) := 0;


"STG_MP_USUARIOS_PAGO_si" NUMBER(22) := 0;

"STG_MP_USUARIOS_PAGO_i" NUMBER(22) := 0;
...
TYPE "T_ROWKEY_EXT_MP_USUARIOS_PAGO" IS TABLE OF VARCHAR2(18) INDEX BY BINARY_INTEGER;
TYPE "T_EXT_MP_USUARIOS_PAGO_2_PIN" IS TABLE OF VARCHAR2(50) INDEX BY BINARY_INTEGER;
TYPE "T_EXT_MP_USUARIOS_PAGO_3_ESTA" IS TABLE OF VARCHAR2(7) INDEX BY BINARY_INTEGER;
TYPE "T_STG_MP_U_0_ID_CLIEN" IS TABLE OF NUMBER(10) INDEX BY BINARY_INTEGER;
TYPE "T_STG_MP_USUARIOS_PAGO_1_PIN" IS TABLE OF VARCHAR2(50) INDEX BY BINARY_INTEGER;
TYPE "T_STG_MP_USUARIOS_PAGO_2_ESTA" IS TABLE OF VARCHAR2(7) INDEX BY BINARY_INTEGER;
...
"SV_ROWKEY_EXT_MP_USUARIOS_PAGO"  VARCHAR2(18);
"SV_EXT_MP_USUARIOS_PAGO_2_PIN"  VARCHAR2(50);
"SV_EXT_MP_USUARIOS_PAGO_3_ESTA"  VARCHAR2(7);
"SV_STG_MP_U_0_ID_CLIEN"  NUMBER(10);
"SV_STG_MP_USUARIOS_PAGO_1_PIN"  VARCHAR2(50);
"SV_STG_MP_USUARIOS_PAGO_2_ESTA"  VARCHAR2(7);
...
"ROWKEY_EXT_MP_USUARIOS_PAGO" "T_ROWKEY_EXT_MP_USUARIOS_PAGO";
"EXT_MP_USUARIOS_PAGO_2_PIN" "T_EXT_MP_USUARIOS_PAGO_2_PIN";
"EXT_MP_USUARIOS_PAGO_3_ESTADO" "T_EXT_MP_USUARIOS_PAGO_3_ESTA";
"STG_MP_U_0_ID_CLIEN" "T_STG_MP_U_0_ID_CLIEN";
"STG_MP_USUARIOS_PAGO_1_PIN" "T_STG_MP_USUARIOS_PAGO_1_PIN";
"STG_MP_USUARIOS_PAGO_2_ESTADO" "T_STG_MP_USUARIOS_PAGO_2_ESTA";
...
---------------------------------------------------------------------------
PROCEDURE "EXT_MP_USUARIOS_PAGO_ES"(error_index IN NUMBER) IS
BEGIN
  ...
  OWBSYS.WB_RT_MAPAUDIT.error_source(
    ...
    p_seq=>1,
    p_instance=>1,
    p_table=>SUBSTR('"EXT_MP_USUARIOS_PAGO"',0,80),
  ...
  OWBSYS.WB_RT_MAPAUDIT.error_source(
    ...
    p_instance=>1,
    p_table=>SUBSTR('"EXT_MP_USUARIOS_PAGO"',0,80),
    p_column=>SUBSTR('PIN',0,80),
    p_value=>SUBSTRB("EXT_MP_USUARIOS_PAGO_2_PIN"(error_index),0,2000),
    p_step=>get_step_number,
    p_role=>'S'
    );
  OWBSYS.WB_RT_MAPAUDIT.error_source(
    ...
    p_instance=>1,
    p_table=>SUBSTR('"EXT_MP_USUARIOS_PAGO"',0,80),
    p_column=>SUBSTR('ESTADO',0,80),
    p_value=>SUBSTRB("EXT_MP_USUARIOS_PAGO_3_ESTADO"(error_index),0,2000),
    p_step=>get_step_number,
    p_role=>'S'
    );
  RETURN;
    
  END "EXT_MP_USUARIOS_PAGO_ES";

---------------------------------------------------------------------------
-- Procedure "EXT_MP_USUARIOS_PAGO_ER" registers error for one erroneous row
---------------------------------------------------------------------------
PROCEDURE "EXT_MP_USUARIOS_PAGO_ER"(p_statement IN VARCHAR2, p_column IN VARCHAR2, p_col_value IN VARCHAR2, p_sqlcode IN NUMBER, p_sqlerrm IN VARCHAR2, p_auditd_id IN NUMBER, p_error_index IN NUMBER) IS
...
END IF;
  ...
  IF ( get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE ) AND (get_rowkey_error > 0) THEN
    "EXT_MP_USUARIOS_PAGO_ES"(p_error_index);
  END IF;
END "EXT_MP_USUARIOS_PAGO_ER";



---------------------------------------------------------------------------
-- Procedure "EXT_MP_USUARIOS_PAGO_SU" opens and initializes data source
-- for map "EXT_MP_USUARIOS_PAGO_p"
---------------------------------------------------------------------------
PROCEDURE "EXT_MP_USUARIOS_PAGO_SU" IS
BEGIN
  IF get_abort THEN
    RETURN;
  END IF;
  IF (NOT "EXT_MP_USUARIOS_PAGO_c"%ISOPEN) THEN
    OPEN "EXT_MP_USUARIOS_PAGO_c";
  END IF;
  get_read_success := TRUE;
END "EXT_MP_USUARIOS_PAGO_SU";

---------------------------------------------------------------------------
-- Procedure "EXT_MP_USUARIOS_PAGO_RD" fetches a bulk of rows from
--   the data source for map "EXT_MP_USUARIOS_PAGO_p"
---------------------------------------------------------------------------
PROCEDURE "EXT_MP_USUARIOS_PAGO_RD" IS
BEGIN
  ...
  BEGIN
    ...
    "EXT_MP_USUARIOS_PAGO_2_PIN".DELETE;
    "EXT_MP_USUARIOS_PAGO_3_ESTADO".DELETE;

    FETCH
      "EXT_MP_USUARIOS_PAGO_c"
    BULK COLLECT INTO
      "EXT_MP_U_1_ID_CLIEN",
      "EXT_MP_USUARIOS_PAGO_2_PIN",
      "EXT_MP_USUARIOS_PAGO_3_ESTADO"
    LIMIT get_bulk_size;

    get_total_processed_rowcount := get_total_processed_rowcount + "EXT_MP_U_1_ID_CLIEN".COUNT;

    IF "EXT_MP_USUARIOS_PAGO_c"%NOTFOUND AND "EXT_MP_U_1_ID_CLIEN".COUNT = 0 THEN
      RETURN;
    END IF;
  ...
  END;
END "EXT_MP_USUARIOS_PAGO_RD";

---------------------------------------------------------------------------
-- Procedure "EXT_MP_USUARIOS_PAGO_DML" does DML for a bulk of rows starting from index si.
---------------------------------------------------------------------------
PROCEDURE "EXT_MP_USUARIOS_PAGO_DML"(si NUMBER, firstround BOOLEAN) IS
 "STG_MP_USUARIOS_PAGO_ins0" NUMBER := "STG_MP_USUARIOS_PAGO_ins";
BEGIN
 IF get_use_hc THEN
  ...
  get_row_status := TRUE;
 END IF;
 IF NOT "STG_MP_USUARIOS_PAGO_St" AND (NOT get_use_hc OR get_row_status) 
AND (NOT (get_use_hc AND "STG_MP_USUARIOS_PAGO_nul")) THEN
  -- Insert DML for "STG_MP_USUARIOS_PAGO"
  normal_action := 'INSERT';
  error_action  := 'INSERT';
  get_target_name := '"STG_MP_USUARIOS_PAGO"';
  get_audit_detail_id := "STG_MP_USUARIOS_PAGO_id";
  IF get_use_hc AND NOT firstround THEN
    "STG_MP_USUARIOS_PAGO_si" := "STG_MP_USUARIOS_PAGO_ir"(si);
    IF "STG_MP_USUARIOS_PAGO_si" = 0 THEN
      "STG_MP_USUARIOS_PAGO_i" := 0;
    ELSE
      "STG_MP_USUARIOS_PAGO_i" := "STG_MP_USUARIOS_PAGO_si" + 1;
    END IF;
  ELSE
    "STG_MP_USUARIOS_PAGO_si" := 1;
  END IF;
  LOOP
    IF NOT get_use_hc THEN
    EXIT WHEN "STG_MP_USUARIOS_PAGO_i" <= get_bulk_size 
 AND "EXT_MP_USUARIOS_PAGO_c"%FOUND AND NOT get_abort;
    ELSE
      EXIT WHEN "STG_MP_USUARIOS_PAGO_si" >= "STG_MP_USUARIOS_PAGO_i";
    END IF;
    get_rowid.DELETE;

    BEGIN
      FORALL i IN "STG_MP_USUARIOS_PAGO_si".."STG_MP_USUARIOS_PAGO_i" - 1
        INSERT 
        /*+ APPEND PARALLEL("STG_MP_USUARIOS_PAGO") */
        INTO
          "STG_MP_USUARIOS_PAGO"
          ("STG_MP_USUARIOS_PAGO"."ID_CLIENTE",
          "STG_MP_USUARIOS_PAGO"."PIN",
          "STG_MP_USUARIOS_PAGO"."ESTADO",
          "STG_MP_USUARIOS_PAGO"."COD_FECHA_DIARIO",
          "STG_MP_USUARIOS_PAGO"."COD_FECHA"
          )
        VALUES
          ("STG_MP_U_0_ID_CLIEN"(i),
          "STG_MP_USUARIOS_PAGO_1_PIN"(i),
          "STG_MP_USUARIOS_PAGO_2_ESTADO"(i),
          "STG_MP_U_3_COD_FECH"(i),
          "STG_MP_U_4_COD_FECH"(i)
          )
        RETURNING ROWID BULK COLLECT INTO get_rowid;
      error_index := "STG_MP_USUARIOS_PAGO_si" + get_rowid.COUNT;
    EXCEPTION
      WHEN OTHERS THEN
          last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
        ...
        error_index := "STG_MP_USUARIOS_PAGO_si" + get_rowid.COUNT;
        IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
          error_rowkey := "STG_MP_USUARIOS_PAGO_srk"(error_index);
          OWBSYS.WB_RT_MAPAUDIT.error(
            p_rta=>get_runtime_audit_id,
            p_step=>get_step_number,
            p_rtd=>get_audit_detail_id,
          ...
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_MP_USUARIOS_PAGO"."ID_CLIENTE"',0,80),SUBSTRB("STG_MP_U_0_ID_CLIEN"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_MP_USUARIOS_PAGO"."PIN"',0,80),SUBSTRB("STG_MP_USUARIOS_PAGO_1_PIN"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_MP_USUARIOS_PAGO"."ESTADO"',0,80),SUBSTRB("STG_MP_USUARIOS_PAGO_2_ESTADO"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_MP_USUARIOS_PAGO"."COD_FECHA_DIARIO"',0,80),SUBSTRB("STG_MP_U_3_COD_FECH"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_MP_USUARIOS_PAGO"."COD_FECHA"',0,80),SUBSTRB("STG_MP_U_4_COD_FECH"(error_index),0,2000));
        ...
        END IF; 
        "STG_MP_USUARIOS_PAGO_err" := "STG_MP_USUARIOS_PAGO_err" + 1;
        
        IF get_errors + "STG_MP_USUARIOS_PAGO_err" > get_max_errors THEN
          get_abort:= TRUE;
        END IF;
    END;
    ...
    IF get_audit_level = AUDIT_COMPLETE THEN
      ...
      rowkey_bulk_index := 1;
      FOR rowkey_index IN "STG_MP_USUARIOS_PAGO_si"..error_index - 1 LOOP
        get_rowkey_bulk(rowkey_bulk_index) := "STG_MP_USUARIOS_PAGO_srk"(rowkey_index);
...
END IF;
    ...
    "STG_MP_USUARIOS_PAGO_ins" := "STG_MP_USUARIOS_PAGO_ins" + get_rowid.COUNT;
    "STG_MP_USUARIOS_PAGO_si" := error_index + 1;

    IF "STG_MP_USUARIOS_PAGO_si" >= "STG_MP_USUARIOS_PAGO_i" OR get_abort THEN
      "STG_MP_USUARIOS_PAGO_i" := 1;
      EXIT;
    END IF;
  END LOOP;
END IF;
...
END IF;
 ...
 "STG_MP_USUARIOS_PAGO_ins" := "STG_MP_USUARIOS_PAGO_ins0";
END IF;

END "EXT_MP_USUARIOS_PAGO_DML";

---------------------------------------------------------------------------
-- "EXT_MP_USUARIOS_PAGO_p" main
---------------------------------------------------------------------------
...
END IF;
  ...
  IF NOT "STG_MP_USUARIOS_PAGO_St" THEN
    ...
    IF get_table_function OR (NOT "EXT_MP_USUARIOS_PAGO_c"%ISOPEN) THEN
      IF NOT (get_audit_level = AUDIT_NONE) THEN
        IF NOT "STG_MP_USUARIOS_PAGO_St" THEN
          "STG_MP_USUARIOS_PAGO_id" :=
...
END IF;
    ...
    "EXT_MP_USUARIOS_PAGO_si" := 0;
    "STG_MP_USUARIOS_PAGO_i" := 1;
    get_rows_processed := FALSE;

    IF NOT get_abort AND NOT get_abort_procedure THEN
      "EXT_MP_USUARIOS_PAGO_SU";

      LOOP
        IF "EXT_MP_USUARIOS_PAGO_si" = 0 THEN
          "EXT_MP_USUARIOS_PAGO_RD";   -- Fetch data from source
          IF NOT get_read_success THEN
          ...
          IF get_use_hc THEN
            dml_bsize := 0;
            "STG_MP_USUARIOS_PAGO_ir".DELETE;
"STG_MP_USUARIOS_PAGO_i" := 1;
          END IF;
        END IF;

        -- Processing:
        "EXT_MP_USUARIOS_PAGO_i" := "EXT_MP_USUARIOS_PAGO_si";
        BEGIN
          
          LOOP
            EXIT WHEN "STG_MP_USUARIOS_PAGO_i" > get_bulk_size OR get_abort OR get_abort_procedure;

            "EXT_MP_USUARIOS_PAGO_i" := "EXT_MP_USUARIOS_PAGO_i" + 1;
            "EXT_MP_USUARIOS_PAGO_si" := "EXT_MP_USUARIOS_PAGO_i";
            IF get_use_hc THEN
              get_row_status := TRUE;
                "STG_MP_USUARIOS_PAGO_new" := FALSE;
            END IF;

            get_buffer_done(get_buffer_done_index) := 
              ("EXT_MP_USUARIOS_PAGO_c"%NOTFOUND AND
               "EXT_MP_USUARIOS_PAGO_i" > bulk_count);

            IF (NOT get_buffer_done(get_buffer_done_index)) AND
              "EXT_MP_USUARIOS_PAGO_i" > bulk_count THEN
...
get_target_name := '"STG_MP_USUARIOS_PAGO"';
            get_audit_detail_id := "STG_MP_USUARIOS_PAGO_id";
            IF NOT "STG_MP_USUARIOS_PAGO_St" AND NOT get_buffer_done(get_buffer_done_index) THEN
              BEGIN
                get_rows_processed := true; -- Set to indicate that some row data was processed (for debugger)
            		error_stmt := SUBSTRB('"STG_MP_U_0_ID_CLIEN"("STG_MP_USUARIOS_PAGO_i") := 
            
            "EXT_MP_U_1_ID_CLIEN"("EXT_MP_USUARIOS_PAGO_i");',0,2000);
            error_column := SUBSTRB('"STG_MP_U_0_ID_CLIEN"',0,80);
            
            BEGIN
              error_value := SUBSTRB("EXT_MP_U_1_ID_CLIEN"("EXT_MP_USUARIOS_PAGO_i"),0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            IF NOT get_use_hc THEN
              "STG_MP_U_0_ID_CLIEN"("STG_MP_USUARIOS_PAGO_i") :=
            
            "EXT_MP_U_1_ID_CLIEN"("EXT_MP_USUARIOS_PAGO_i");
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_MP_USUARIOS_PAGO_1_PIN"("STG_MP_USUARIOS_PAGO_i") := 
            
            "EXT_MP_USUARIOS_PAGO_2_PIN"("EXT_MP_USUARIOS_PAGO_i");',0,2000);
            error_column := SUBSTRB('"STG_MP_USUARIOS_PAGO_1_PIN"',0,80);
            
            BEGIN
              error_value := SUBSTRB("EXT_MP_USUARIOS_PAGO_2_PIN"("EXT_MP_USUARIOS_PAGO_i"),0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            IF NOT get_use_hc THEN
              "STG_MP_USUARIOS_PAGO_1_PIN"("STG_MP_USUARIOS_PAGO_i") :=
            
            "EXT_MP_USUARIOS_PAGO_2_PIN"("EXT_MP_USUARIOS_PAGO_i");
            
            ELSIF get_row_status THEN
              "SV_STG_MP_USUARIOS_PAGO_1_PIN" :=
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_MP_USUARIOS_PAGO_2_ESTADO"("STG_MP_USUARIOS_PAGO_i") := 
            
            "EXT_MP_USUARIOS_PAGO_3_ESTADO"("EXT_MP_USUARIOS_PAGO_i");',0,2000);
            error_column := SUBSTRB('"STG_MP_USUARIOS_PAGO_2_ESTADO"',0,80);
            
            BEGIN
              error_value := SUBSTRB("EXT_MP_USUARIOS_PAGO_3_ESTADO"("EXT_MP_USUARIOS_PAGO_i"),0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            IF NOT get_use_hc THEN
              "STG_MP_USUARIOS_PAGO_2_ESTADO"("STG_MP_USUARIOS_PAGO_i") :=
            
            "EXT_MP_USUARIOS_PAGO_3_ESTADO"("EXT_MP_USUARIOS_PAGO_i");
            
            ELSIF get_row_status THEN
              "SV_STG_MP_USUARIOS_PAGO_2_ESTA" :=
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_MP_U_3_COD_FECH"("STG_MP_USUARIOS_PAGO_i") := 
            
            "APP_STG_MP_USUARIOS_PAGO"."COD_FECHA_DIARIO"/* ATTRIBUTE P_COD_FECHA_DIARIO.OUTGRP1.COD_FECHA_DIARIO */;',0,2000);
            error_column := SUBSTRB('"STG_MP_U_3_COD_FECH"',0,80);
            
            BEGIN
              error_value := SUBSTRB("APP_STG_MP_USUARIOS_PAGO"."COD_FECHA_DIARIO"/* ATTRIBUTE P_COD_FECHA_DIARIO.OUTGRP1.COD_FECHA_DIARIO */,0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            IF NOT get_use_hc THEN
              "STG_MP_U_3_COD_FECH"("STG_MP_USUARIOS_PAGO_i") :=
            
            "APP_STG_MP_USUARIOS_PAGO"."COD_FECHA_DIARIO"/* ATTRIBUTE P_COD_FECHA_DIARIO.OUTGRP1.COD_FECHA_DIARIO */;
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_MP_U_4_COD_FECH"("STG_MP_USUARIOS_PAGO_i") := 
            
            "APP_STG_MP_USUARIOS_PAGO"."COD_FECHA"/* ATTRIBUTE P_COD_FECHA.OUTGRP1.COD_FECHA */;',0,2000);
            error_column := SUBSTRB('"STG_MP_U_4_COD_FECH"',0,80);
            
            BEGIN
              error_value := SUBSTRB("APP_STG_MP_USUARIOS_PAGO"."COD_FECHA"/* ATTRIBUTE P_COD_FECHA.OUTGRP1.COD_FECHA */,0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            IF NOT get_use_hc THEN
              "STG_MP_U_4_COD_FECH"("STG_MP_USUARIOS_PAGO_i") :=
            
            "APP_STG_MP_USUARIOS_PAGO"."COD_FECHA"/* ATTRIBUTE P_COD_FECHA.OUTGRP1.COD_FECHA */;
            ...
            END IF;
                ...
                IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
                  IF NOT get_use_hc THEN
                    "STG_MP_USUARIOS_PAGO_srk"("STG_MP_USUARIOS_PAGO_i") := get_rowkey + "EXT_MP_USUARIOS_PAGO_i" - 1;
                  ELSIF get_row_status THEN
                    "SV_STG_MP_USUARIOS_PAGO_srk" := get_rowkey + "EXT_MP_USUARIOS_PAGO_i" - 1;
                  ELSE
                    NULL;
                  END IF;
                  END IF;
                  IF get_use_hc THEN
                  "STG_MP_USUARIOS_PAGO_new" := TRUE;
                ELSE
                  "STG_MP_USUARIOS_PAGO_i" := "STG_MP_USUARIOS_PAGO_i" + 1;
                END IF;
              EXCEPTION
                WHEN OTHERS THEN
                    last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
                  ...
                  "EXT_MP_USUARIOS_PAGO_ER"('TRACE 22: ' || error_stmt, error_column, error_value, SQLCODE, SQLERRM, get_audit_detail_id, "EXT_MP_USUARIOS_PAGO_i");
            ...
            IF get_use_hc AND get_row_status AND ("STG_MP_USUARIOS_PAGO_new") THEN
              dml_bsize := dml_bsize + 1;
            	IF "STG_MP_USUARIOS_PAGO_new" 
            AND (NOT "STG_MP_USUARIOS_PAGO_nul") THEN
              "STG_MP_USUARIOS_PAGO_ir"(dml_bsize) := "STG_MP_USUARIOS_PAGO_i";
            	"STG_MP_U_0_ID_CLIEN"("STG_MP_USUARIOS_PAGO_i") := "SV_STG_MP_U_0_ID_CLIEN";
            	"STG_MP_USUARIOS_PAGO_1_PIN"("STG_MP_USUARIOS_PAGO_i") := "SV_STG_MP_USUARIOS_PAGO_1_PIN";
            	"STG_MP_USUARIOS_PAGO_2_ESTADO"("STG_MP_USUARIOS_PAGO_i") := "SV_STG_MP_USUARIOS_PAGO_2_ESTA";
            	"STG_MP_U_3_COD_FECH"("STG_MP_USUARIOS_PAGO_i") := "SV_STG_MP_U_3_COD_FECH";
            	"STG_MP_U_4_COD_FECH"("STG_MP_USUARIOS_PAGO_i") := "SV_STG_MP_U_4_COD_FECH";
              "STG_MP_USUARIOS_PAGO_srk"("STG_MP_USUARIOS_PAGO_i") := "SV_STG_MP_USUARIOS_PAGO_srk";
              "STG_MP_USUARIOS_PAGO_i" := "STG_MP_USUARIOS_PAGO_i" + 1;
            ELSE
              "STG_MP_USUARIOS_PAGO_ir"(dml_bsize) := 0;
            END IF;
            END IF;
            
          END LOOP;

          "EXT_MP_USUARIOS_PAGO_DML"(1, TRUE);
          IF get_use_hc THEN
            IF NOT get_row_status THEN
              FOR start_index IN 1..dml_bsize LOOP
                "EXT_MP_USUARIOS_PAGO_DML"(start_index, FALSE);
              END LOOP;
            ELSE
              COMMIT;
            END IF;
        ...
        EXCEPTION
          WHEN OTHERS THEN
              last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
            "EXT_MP_USUARIOS_PAGO_ER"('TRACE 19: ' || error_stmt, '*', NULL, SQLCODE, SQLERRM, NULL, "EXT_MP_USUARIOS_PAGO_i");
            get_errors := get_errors + 1;
...
END IF;
  ...
  IF NOT "STG_MP_USUARIOS_PAGO_St" AND bulk_commit_count > get_commit_frequency THEN
            IF NOT (get_audit_level = AUDIT_NONE) THEN
              OWBSYS.WB_RT_MAPAUDIT.auditd_progress(
                p_rtd=>"STG_MP_USUARIOS_PAGO_id",
                p_sel=>get_map_selected,
                p_ins=>"STG_MP_USUARIOS_PAGO_ins",
                p_upd=>"STG_MP_USUARIOS_PAGO_upd",
                p_del=>"STG_MP_USUARIOS_PAGO_del",
                p_err=>"STG_MP_USUARIOS_PAGO_err",
                p_dis=>NULL
              );
            END IF;
            IF NOT get_no_commit THEN
        ...
        cursor_exhausted := "EXT_MP_USUARIOS_PAGO_c"%NOTFOUND;
        exit_loop_normal := get_abort OR get_abort_procedure OR (cursor_exhausted AND "EXT_MP_USUARIOS_PAGO_i" > bulk_count);
        exit_loop_early := get_rows_processed AND get_bulk_loop_count IS NOT NULL AND "EXT_MP_USUARIOS_PAGO_i" >= get_bulk_loop_count;
  ...
  IF NOT "STG_MP_USUARIOS_PAGO_St"
    ...
    END IF;
  	get_inserted := get_inserted + "STG_MP_USUARIOS_PAGO_ins";
    get_updated  := get_updated  + "STG_MP_USUARIOS_PAGO_upd";
    get_deleted  := get_deleted  + "STG_MP_USUARIOS_PAGO_del";
    get_errors   := get_errors   + "STG_MP_USUARIOS_PAGO_err";
...
END "EXT_MP_USUARIOS_PAGO_p";



-- Procedure "EXT_MP_USUARIOS_PAGO_t" is the entry point for map "EXT_MP_USUARIOS_PAGO_t"

PROCEDURE "EXT_MP_USUARIOS_PAGO_t"
("CANT_FILAS" IN NUMBER
 DEFAULT NULL,
"TABLE_NAME" IN VARCHAR2
 DEFAULT NULL,
...
"COD_FECHA_DIARIO" IN NUMBER
 DEFAULT NULL) IS
-- Constants for this map
get_map_name               CONSTANT VARCHAR2(40) := '"EXT_MP_USUARIOS_PAGO_t"';
get_source_name            CONSTANT VARCHAR2(2000) := SUBSTRB('"EXT_MP_USUARIOS_PAGO"',0,2000);
get_source_uoid            CONSTANT VARCHAR2(2000) := SUBSTRB('04641062EC9C2D98E053B200090AD236',0,2000);
get_step_number            CONSTANT NUMBER(22) := 1;
...
"EXT_MP_USUARIOS_PAGO_si$1" NUMBER(22) := 0;

"EXT_MP_USUARIOS_PAGO_i$1" NUMBER(22) := 0;
...
TYPE "T_EXT_MP_USUARIOS_PAGO_2_PIN$1" IS TABLE OF VARCHAR2(50) INDEX BY BINARY_INTEGER;
TYPE "T_EXT_MP_USUARIOS_PAGO_3_ES$1" IS TABLE OF VARCHAR2(7) INDEX BY BINARY_INTEGER;
...
TYPE "T_STG_MP_USUARIOS_PAGO_1_PIN$1" IS TABLE OF VARCHAR2(50) INDEX BY BINARY_INTEGER;
TYPE "T_STG_MP_USUARIOS_PAGO_2_ES$1" IS TABLE OF VARCHAR2(7) INDEX BY BINARY_INTEGER;
...
"SV_EXT_MP_USUARIOS_PAGO_2_P$1"  VARCHAR2(50);
"SV_EXT_MP_USUARIOS_PAGO_3_ES$1"  VARCHAR2(7);
"SV_ROWKEY_DUMMY_TABLE_CURSOR"  VARCHAR2(18);
"SV_STG_MP_U_0_ID_CLIEN$1"  NUMBER(10);
"SV_STG_MP_USUARIOS_PAGO_1_P$1"  VARCHAR2(50);
"SV_STG_MP_USUARIOS_PAGO_2_ES$1"  VARCHAR2(7);
"SV_STG_MP_U_3_COD_FECH$1"  NUMBER(8);
"SV_STG_MP_U_4_COD_FECH$1"  NUMBER(8);

-- Bulk: intermediate collection variables
"EXT_MP_U_1_ID_CLIEN$1" "T_EXT_MP_U_1_ID_CLIEN$1";
"ROWKEY_EXT_MP_USUARIOS_PAGO$1" "T_ROWKEY_EXT_MP_USUARIOS_PA$1";
"EXT_MP_USUARIOS_PAGO_2_PIN$1" "T_EXT_MP_USUARIOS_PAGO_2_PIN$1";
"EXT_MP_USUARIOS_PAGO_3_ESTA$1" "T_EXT_MP_USUARIOS_PAGO_3_ES$1";
"ROWKEY_DUMMY_TABLE_CURSOR" "T_ROWKEY_DUMMY_TABLE_CURSOR";
"STG_MP_U_0_ID_CLIEN$1" "T_STG_MP_U_0_ID_CLIEN$1";
"STG_MP_USUARIOS_PAGO_1_PIN$1" "T_STG_MP_USUARIOS_PAGO_1_PIN$1";
"STG_MP_USUARIOS_PAGO_2_ESTA$1" "T_STG_MP_USUARIOS_PAGO_2_ES$1";
...
--    RTE_ROWKEY, uniquely identifies a source row within and audited run
---------------------------------------------------------------------------
PROCEDURE "EXT_MP_USUARIOS_PAGO_ES$1"(error_index IN NUMBER) IS
BEGIN
  ...
  OWBSYS.WB_RT_MAPAUDIT.error_source(
    ...
    p_table=>SUBSTR('"EXT_MP_USUARIOS_PAGO"',0,80),
    p_column=>SUBSTR('PIN',0,80),
    p_value=>SUBSTRB("EXT_MP_USUARIOS_PAGO_2_PIN$1"(error_index),0,2000),
    p_step=>get_step_number,
    p_role=>'S'
    );
  OWBSYS.WB_RT_MAPAUDIT.error_source(
    ...
    p_table=>SUBSTR('"EXT_MP_USUARIOS_PAGO"',0,80),
    p_column=>SUBSTR('ESTADO',0,80),
    p_value=>SUBSTRB("EXT_MP_USUARIOS_PAGO_3_ESTA$1"(error_index),0,2000),
    p_step=>get_step_number,
    p_role=>'S'
    );
  RETURN;
    
  END "EXT_MP_USUARIOS_PAGO_ES$1";

---------------------------------------------------------------------------
-- Procedure "EXT_MP_USUARIOS_PAGO_ER$1" registers error for one erroneous row
---------------------------------------------------------------------------
PROCEDURE "EXT_MP_USUARIOS_PAGO_ER$1"(p_statement IN VARCHAR2, p_column IN VARCHAR2, p_col_value IN VARCHAR2, p_sqlcode IN NUMBER, p_sqlerrm IN VARCHAR2, p_auditd_id IN NUMBER, p_error_index IN NUMBER) IS
l_source_target_name VARCHAR2(80);
BEGIN
  IF p_auditd_id IS NULL THEN
    l_source_target_name := SUBSTR(get_source_name,0,80);
...
END IF;
  ...
  IF ( get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE ) AND (get_rowkey_error > 0) THEN
    "EXT_MP_USUARIOS_PAGO_ES$1"(p_error_index);
  END IF;
END "EXT_MP_USUARIOS_PAGO_ER$1";



---------------------------------------------------------------------------
-- Procedure "EXT_MP_USUARIOS_PAGO_SU$1" opens and initializes data source
-- for map "EXT_MP_USUARIOS_PAGO_t"
---------------------------------------------------------------------------
PROCEDURE "EXT_MP_USUARIOS_PAGO_SU$1" IS
BEGIN
  IF get_abort THEN
    RETURN;
  END IF;
  IF (NOT "EXT_MP_USUARIOS_PAGO_c$1"%ISOPEN) THEN
    OPEN "EXT_MP_USUARIOS_PAGO_c$1";
  END IF;
  get_read_success := TRUE;
END "EXT_MP_USUARIOS_PAGO_SU$1";

---------------------------------------------------------------------------
-- Procedure "EXT_MP_USUARIOS_PAGO_RD$1" fetches a bulk of rows from
--   the data source for map "EXT_MP_USUARIOS_PAGO_t"
---------------------------------------------------------------------------
PROCEDURE "EXT_MP_USUARIOS_PAGO_RD$1" IS
BEGIN
  IF NOT get_read_success THEN
    get_abort := TRUE;
    IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
  ...
  BEGIN
    ...
    "EXT_MP_USUARIOS_PAGO_2_PIN$1".DELETE;
    "EXT_MP_USUARIOS_PAGO_3_ESTA$1".DELETE;

    FETCH
      "EXT_MP_USUARIOS_PAGO_c$1"
    BULK COLLECT INTO
      "EXT_MP_U_1_ID_CLIEN$1",
      "EXT_MP_USUARIOS_PAGO_2_PIN$1",
      "EXT_MP_USUARIOS_PAGO_3_ESTA$1"
    LIMIT get_bulk_size;

    get_total_processed_rowcount := get_total_processed_rowcount + "EXT_MP_U_1_ID_CLIEN$1".COUNT;

    IF "EXT_MP_USUARIOS_PAGO_c$1"%NOTFOUND AND "EXT_MP_U_1_ID_CLIEN$1".COUNT = 0 THEN
      RETURN;
    END IF;
    -- register feedback for successful reads
    IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
  ...
  last_error_message := SQLERRM;
      ...
      IF get_errors > get_max_errors THEN get_abort := TRUE; END IF;
  END;
END "EXT_MP_USUARIOS_PAGO_RD$1";

---------------------------------------------------------------------------
-- Procedure "EXT_MP_USUARIOS_PAGO_DML$1" does DML for a bulk of rows starting from index si.
---------------------------------------------------------------------------
PROCEDURE "EXT_MP_USUARIOS_PAGO_DML$1"(si NUMBER, firstround BOOLEAN) IS
 "STG_MP_USUARIOS_PAGO_ins0" NUMBER := "STG_MP_USUARIOS_PAGO_ins";
BEGIN
 IF get_use_hc THEN
  IF firstround AND NOT get_row_status THEN
...
AND (NOT (get_use_hc AND "STG_MP_USUARIOS_PAGO_nul")) THEN
  ...
  LOOP
    IF NOT get_use_hc THEN
    EXIT WHEN "STG_MP_USUARIOS_PAGO_i" <= get_bulk_size 
 AND "EXT_MP_USUARIOS_PAGO_c$1"%FOUND AND NOT get_abort;
    ELSE
      EXIT WHEN "STG_MP_USUARIOS_PAGO_si" >= "STG_MP_USUARIOS_PAGO_i";
    END IF;
    get_rowid.DELETE;
    ...
    BEGIN
      FORALL i IN "STG_MP_USUARIOS_PAGO_si".."STG_MP_USUARIOS_PAGO_i" - 1
        ...
        VALUES
          ...
          "STG_MP_USUARIOS_PAGO_1_PIN$1"(i),
          "STG_MP_USUARIOS_PAGO_2_ESTA$1"(i),
  ...
  last_error_message := SQLERRM;
        ...
        IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
          ...
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_MP_USUARIOS_PAGO"."ID_CLIENTE"',0,80),SUBSTRB("STG_MP_U_0_ID_CLIEN$1"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_MP_USUARIOS_PAGO"."PIN"',0,80),SUBSTRB("STG_MP_USUARIOS_PAGO_1_PIN$1"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_MP_USUARIOS_PAGO"."ESTADO"',0,80),SUBSTRB("STG_MP_USUARIOS_PAGO_2_ESTA$1"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_MP_USUARIOS_PAGO"."COD_FECHA_DIARIO"',0,80),SUBSTRB("STG_MP_U_3_COD_FECH$1"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_MP_USUARIOS_PAGO"."COD_FECHA"',0,80),SUBSTRB("STG_MP_U_4_COD_FECH$1"(error_index),0,2000));
...
END "EXT_MP_USUARIOS_PAGO_DML$1";

---------------------------------------------------------------------------
-- "EXT_MP_USUARIOS_PAGO_t" main
---------------------------------------------------------------------------
...
END IF;
  ...
  IF NOT "STG_MP_USUARIOS_PAGO_St" THEN
    ...
    -- For table function (parallel row mode) operation, the cursor will already be
    -- open when the procedure is called, so execute the initialization.
    IF get_table_function OR (NOT "EXT_MP_USUARIOS_PAGO_c$1"%ISOPEN) THEN
      IF NOT (get_audit_level = AUDIT_NONE) THEN
        IF NOT "STG_MP_USUARIOS_PAGO_St" THEN
          "STG_MP_USUARIOS_PAGO_id" :=
            OWBSYS.WB_RT_MAPAUDIT.auditd_begin(  -- Template AuditDetailBegin
...
END IF;
    ...
    -- Initialize internal loop index variables
    "EXT_MP_USUARIOS_PAGO_si$1" := 0;
    "STG_MP_USUARIOS_PAGO_i" := 1;
    get_rows_processed := FALSE;

    IF NOT get_abort AND NOT get_abort_procedure THEN
      "EXT_MP_USUARIOS_PAGO_SU$1";

      LOOP
        IF "EXT_MP_USUARIOS_PAGO_si$1" = 0 THEN
          "EXT_MP_USUARIOS_PAGO_RD$1";   -- Fetch data from source
          IF NOT get_read_success THEN
...
"STG_MP_USUARIOS_PAGO_i" := 1;
        ...
        -- Processing:
        "EXT_MP_USUARIOS_PAGO_i$1" := "EXT_MP_USUARIOS_PAGO_si$1";
        BEGIN
          
          LOOP
            EXIT WHEN "STG_MP_USUARIOS_PAGO_i" > get_bulk_size OR get_abort OR get_abort_procedure;

            "EXT_MP_USUARIOS_PAGO_i$1" := "EXT_MP_USUARIOS_PAGO_i$1" + 1;
            "EXT_MP_USUARIOS_PAGO_si$1" := "EXT_MP_USUARIOS_PAGO_i$1";
            IF get_use_hc THEN
              get_row_status := TRUE;
                "STG_MP_USUARIOS_PAGO_new" := FALSE;
            END IF;
            ...
            get_buffer_done(get_buffer_done_index) := 
              ("EXT_MP_USUARIOS_PAGO_c$1"%NOTFOUND AND
               "EXT_MP_USUARIOS_PAGO_i$1" > bulk_count);

            IF (NOT get_buffer_done(get_buffer_done_index)) AND
              "EXT_MP_USUARIOS_PAGO_i$1" > bulk_count THEN
...
get_target_name := '"STG_MP_USUARIOS_PAGO"';
            ...
            IF NOT "STG_MP_USUARIOS_PAGO_St" AND NOT get_buffer_done(get_buffer_done_index) THEN
              BEGIN
                get_rows_processed := true; -- Set to indicate that some row data was processed (for debugger)
            		error_stmt := SUBSTRB('"STG_MP_U_0_ID_CLIEN$1"("STG_MP_USUARIOS_PAGO_i") := 
            
            "EXT_MP_U_1_ID_CLIEN$1"("EXT_MP_USUARIOS_PAGO_i$1");',0,2000);
            error_column := SUBSTRB('"STG_MP_U_0_ID_CLIEN$1"',0,80);
            
            BEGIN
              error_value := SUBSTRB("EXT_MP_U_1_ID_CLIEN$1"("EXT_MP_USUARIOS_PAGO_i$1"),0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            IF NOT get_use_hc THEN
              "STG_MP_U_0_ID_CLIEN$1"("STG_MP_USUARIOS_PAGO_i") :=
            
            "EXT_MP_U_1_ID_CLIEN$1"("EXT_MP_USUARIOS_PAGO_i$1");
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_MP_USUARIOS_PAGO_1_PIN$1"("STG_MP_USUARIOS_PAGO_i") := 
            
            "EXT_MP_USUARIOS_PAGO_2_PIN$1"("EXT_MP_USUARIOS_PAGO_i$1");',0,2000);
            error_column := SUBSTRB('"STG_MP_USUARIOS_PAGO_1_PIN$1"',0,80);
            
            BEGIN
              error_value := SUBSTRB("EXT_MP_USUARIOS_PAGO_2_PIN$1"("EXT_MP_USUARIOS_PAGO_i$1"),0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            IF NOT get_use_hc THEN
              "STG_MP_USUARIOS_PAGO_1_PIN$1"("STG_MP_USUARIOS_PAGO_i") :=
            
            "EXT_MP_USUARIOS_PAGO_2_PIN$1"("EXT_MP_USUARIOS_PAGO_i$1");
            
            ELSIF get_row_status THEN
              "SV_STG_MP_USUARIOS_PAGO_1_P$1" :=
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_MP_USUARIOS_PAGO_2_ESTA$1"("STG_MP_USUARIOS_PAGO_i") := 
            
            "EXT_MP_USUARIOS_PAGO_3_ESTA$1"("EXT_MP_USUARIOS_PAGO_i$1");',0,2000);
            error_column := SUBSTRB('"STG_MP_USUARIOS_PAGO_2_ESTA$1"',0,80);
            
            BEGIN
              error_value := SUBSTRB("EXT_MP_USUARIOS_PAGO_3_ESTA$1"("EXT_MP_USUARIOS_PAGO_i$1"),0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            IF NOT get_use_hc THEN
              "STG_MP_USUARIOS_PAGO_2_ESTA$1"("STG_MP_USUARIOS_PAGO_i") :=
            
            "EXT_MP_USUARIOS_PAGO_3_ESTA$1"("EXT_MP_USUARIOS_PAGO_i$1");
            
            ELSIF get_row_status THEN
              "SV_STG_MP_USUARIOS_PAGO_2_ES$1" :=
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_MP_U_3_COD_FECH$1"("STG_MP_USUARIOS_PAGO_i") := 
            ...
            IF NOT get_use_hc THEN
              "STG_MP_U_3_COD_FECH$1"("STG_MP_USUARIOS_PAGO_i") :=
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_MP_U_4_COD_FECH$1"("STG_MP_USUARIOS_PAGO_i") := 
            ...
            IF NOT get_use_hc THEN
              "STG_MP_U_4_COD_FECH$1"("STG_MP_USUARIOS_PAGO_i") :=
            ...
            END IF;
                ...
                IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
                  IF NOT get_use_hc THEN
                    "STG_MP_USUARIOS_PAGO_srk"("STG_MP_USUARIOS_PAGO_i") := get_rowkey + "EXT_MP_USUARIOS_PAGO_i$1" - 1;
                  ELSIF get_row_status THEN
                    "SV_STG_MP_USUARIOS_PAGO_srk" := get_rowkey + "EXT_MP_USUARIOS_PAGO_i$1" - 1;
                  ELSE
                    NULL;
                  END IF;
                  END IF;
  ...
  last_error_message := SQLERRM;
                  ...
                  "EXT_MP_USUARIOS_PAGO_ER$1"('TRACE 29: ' || error_stmt, error_column, error_value, SQLCODE, SQLERRM, get_audit_detail_id, "EXT_MP_USUARIOS_PAGO_i$1");
            ...
            AND (NOT "STG_MP_USUARIOS_PAGO_nul") THEN
              "STG_MP_USUARIOS_PAGO_ir"(dml_bsize) := "STG_MP_USUARIOS_PAGO_i";
            	"STG_MP_U_0_ID_CLIEN$1"("STG_MP_USUARIOS_PAGO_i") := "SV_STG_MP_U_0_ID_CLIEN$1";
            	"STG_MP_USUARIOS_PAGO_1_PIN$1"("STG_MP_USUARIOS_PAGO_i") := "SV_STG_MP_USUARIOS_PAGO_1_P$1";
            	"STG_MP_USUARIOS_PAGO_2_ESTA$1"("STG_MP_USUARIOS_PAGO_i") := "SV_STG_MP_USUARIOS_PAGO_2_ES$1";
            	"STG_MP_U_3_COD_FECH$1"("STG_MP_USUARIOS_PAGO_i") := "SV_STG_MP_U_3_COD_FECH$1";
            	"STG_MP_U_4_COD_FECH$1"("STG_MP_USUARIOS_PAGO_i") := "SV_STG_MP_U_4_COD_FECH$1";
              "STG_MP_USUARIOS_PAGO_srk"("STG_MP_USUARIOS_PAGO_i") := "SV_STG_MP_USUARIOS_PAGO_srk";
              "STG_MP_USUARIOS_PAGO_i" := "STG_MP_USUARIOS_PAGO_i" + 1;
            ELSE
              "STG_MP_USUARIOS_PAGO_ir"(dml_bsize) := 0;
          ...
          "EXT_MP_USUARIOS_PAGO_DML$1"(1, TRUE);
          IF get_use_hc THEN
            IF NOT get_row_status THEN
              FOR start_index IN 1..dml_bsize LOOP
                "EXT_MP_USUARIOS_PAGO_DML$1"(start_index, FALSE);
              END LOOP;
            ELSE
              COMMIT;
            END IF;
        ...
        EXCEPTION
          WHEN OTHERS THEN
              last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
            "EXT_MP_USUARIOS_PAGO_ER$1"('TRACE 26: ' || error_stmt, '*', NULL, SQLCODE, SQLERRM, NULL, "EXT_MP_USUARIOS_PAGO_i$1");
            get_errors := get_errors + 1;
            IF get_errors > get_max_errors THEN  
  get_abort := TRUE;
END IF;
  ...
  IF NOT "STG_MP_USUARIOS_PAGO_St" AND bulk_commit_count > get_commit_frequency THEN
        ...
        cursor_exhausted := "EXT_MP_USUARIOS_PAGO_c$1"%NOTFOUND;
        exit_loop_normal := get_abort OR get_abort_procedure OR (cursor_exhausted AND "EXT_MP_USUARIOS_PAGO_i$1" > bulk_count);
        exit_loop_early := get_rows_processed AND get_bulk_loop_count IS NOT NULL AND "EXT_MP_USUARIOS_PAGO_i$1" >= get_bulk_loop_count;
        get_close_cursor := get_abort OR get_abort_procedure OR cursor_exhausted;
        EXIT WHEN exit_loop_normal OR exit_loop_early;
    ...
    BEGIN
      IF get_close_cursor THEN
        CLOSE "EXT_MP_USUARIOS_PAGO_c$1";
      END IF;
    EXCEPTION WHEN OTHERS THEN
      NULL;
      END;
...
PROCEDURE Initialize("CANT_FILAS" IN NUMBER
 DEFAULT NULL,
"TABLE_NAME" IN VARCHAR2
 DEFAULT NULL,
"COD_FECHA" IN NUMBER
...
"APP_STG_MP_USUARIOS_PAGO"."CANT_FILAS" := "APP_STG_MP_USUARIOS_PAGO".Initialize."CANT_FILAS";
"APP_STG_MP_USUARIOS_PAGO"."TABLE_NAME" := "APP_STG_MP_USUARIOS_PAGO".Initialize."TABLE_NAME";
"APP_STG_MP_USUARIOS_PAGO"."COD_FECHA" := "APP_STG_MP_USUARIOS_PAGO".Initialize."COD_FECHA";
"APP_STG_MP_USUARIOS_PAGO"."COD_FECHA_DIARIO" := "APP_STG_MP_USUARIOS_PAGO".Initialize."COD_FECHA_DIARIO";
  ...
  IF NOT (get_audit_level = AUDIT_NONE) THEN
    get_runtime_audit_id := OWBSYS.WB_RT_MAPAUDIT.audit_begin(  -- Template AuditBegin
      ...
      p_parent=>NULL,
      p_source=>'"EXT_MP_USUARIOS_PAGO"',
      p_source_uoid=>'04641062EC9C2D98E053B200090AD236',
      p_target=>'"STG_MP_USUARIOS_PAGO"',
      p_target_uoid=>'04641062EBF22D98E053B200090AD236',      p_info=>NULL,
...
BEGIN
  ...
  "PACK_STG_UTILS"."FN_TRAILER_CHECK"(("APP_STG_MP_USUARIOS_PAGO"."TABLE_NAME"/* ATTRIBUTE P_TABLE_NAME.OUTGRP1.TABLE_NAME */),("APP_STG_MP_USUARIOS_PAGO"."CANT_FILAS"/* ATTRIBUTE P_CANT_FILAS.OUTGRP1.CANT_FILAS */))/* OPERATOR FN_TRAILER_CHECK */;
...
PROCEDURE Finalize("CANT_FILAS" IN NUMBER
 DEFAULT NULL,
"TABLE_NAME" IN VARCHAR2
 DEFAULT NULL,
"COD_FECHA" IN NUMBER
...
BEGIN
  ...
  END IF;
        ...
        AND    uo.object_name = 'APP_STG_MP_USUARIOS_PAGO'
        AND    uo.object_id = ao.object_id;
 
        OWBSYS.wb_rt_mapaudit_util.set_schema_workspace(x_schema);
    
  Initialize("CANT_FILAS", "TABLE_NAME", "COD_FECHA", "COD_FECHA_DIARIO", 
  p_env);
  ...
  END IF;
   ...
   IF get_operating_mode = MODE_SET THEN
    ...
    IF NOT get_use_hc OR get_batch_status THEN
        "STG_MP_USUARIOS_PAGO_St" := "STG_MP_USUARIOS_PAGO_Bat"("CANT_FILAS", "TABLE_NAME", "COD_FECHA", "COD_FECHA_DIARIO");
        get_batch_status := get_batch_status AND "STG_MP_USUARIOS_PAGO_St";
      END IF;
   ...
   "EXT_MP_USUARIOS_PAGO_p"("CANT_FILAS", "TABLE_NAME", "COD_FECHA", "COD_FECHA_DIARIO");
   END IF;
  ...
  "EXT_MP_USUARIOS_PAGO_t"("CANT_FILAS", "TABLE_NAME", "COD_FECHA", "COD_FECHA_DIARIO");
  ...
  END IF;
   ...
   Finalize("CANT_FILAS", "TABLE_NAME", "COD_FECHA", "COD_FECHA_DIARIO", 
  p_env);
...
BEGIN
  ...
  IF get_job_audit THEN 
     OWBSYS.wb_rt_mapaudit_util.premap('APP_STG_MP_USUARIOS_PAGO', x_schema, x_audit_id, x_object_id);
  ELSE
    OWBSYS.wb_rt_mapaudit_util.set_schema_workspace(x_schema);
  END IF;
...
-- OWBSYS.wb_rt_mapaudit_util.register_custom_param(x_audit_id,
  ...
  x_return_code := NULL;
  BEGIN
      x_result := Main( "CANT_FILAS", "TABLE_NAME", "COD_FECHA", "COD_FECHA_DIARIO",
      x_env);
  EXCEPTION
    WHEN OTHERS THEN
      x_result := 1;
...
BEGIN
BEGIN
  IF "EXT_MP_USUARIOS_PAGO_c"%ISOPEN THEN
    CLOSE "EXT_MP_USUARIOS_PAGO_c";
  END IF;
EXCEPTION
  WHEN OTHERS THEN
    NULL;    
END;BEGIN
  IF "EXT_MP_USUARIOS_PAGO_c$1"%ISOPEN THEN
    CLOSE "EXT_MP_USUARIOS_PAGO_c$1";
  END IF;
EXCEPTION
  WHEN OTHERS THEN
